...
class Environment(Process):
    def __init__(self, env_idx, child_conn, env_name, state_size, action_size, visualize=True):
...
class Actor_Model:
    ...
    def ppo_loss(self, y_true, y_pred):
        # Defined in https://arxiv.org/abs/1707.06347 (nothing usefull)
        advantages, prediction_picks, actions = y_true[:, :1], y_true[:, 1:1+self.action_space], y_true[:, 1+self.action_space:]
        LOSS_CLIPPING = 0.2 # clipping range (0.2 suggested by the papper). Means the 20% change of policy only allowed (to positive or negative dirrection)
        ENTROPY_LOSS = 0.001
        
        prob = actions * y_pred # y_pred - newpolicy_prob
        old_prob = actions * prediction_picks # prediction_picks - old policy probabilities

        prob = K.clip(prob, 1e-10, 1.0)
        old_prob = K.clip(old_prob, 1e-10, 1.0)

        # calculate how much the policy has changed (in log form)
        ratio = K.exp(K.log(prob) - K.log(old_prob))
        
        # actor loss is the minimum of two functions
        p1 = ratio * advantages
        # p2 is clipped version of p1.
        p2 = K.clip(ratio, min_value=1 - LOSS_CLIPPING, max_value=1 + LOSS_CLIPPING) * advantages
        actor_loss = -K.mean(K.minimum(p1, p2))

        entropy = -(y_pred * K.log(y_pred + 1e-10)) # entropy - encourages the model to explore different policies  (about 0.001?)
        entropy = ENTROPY_LOSS * K.mean(entropy)    # optional parameter
...
class Critic_Model:
    ...
    def critic_PPO2_loss(self, values):
        def loss(y_true, y_pred):
            ...
            #value_loss = K.mean((y_true - y_pred) ** 2) # standard PPO loss. Mean Squared Error - mse
            return value_loss
        return loss
...
class PPOAgent:
    ...
    # calculates advantage. Advantage is the measure of
    # how much better off can be by taking a particular action
    # when in a particular state.
    def get_gaes(self, rewards, dones, values, next_values, gamma = 0.99, lamda = 0.9, normalize=True):
        # get delta
        # gamma - discount factor (from 0 to 1). The more gamma the more long period rewards oriented.
        # 1-d - mask
        deltas = [r + gamma * (1 - d) * nv - v for r, d, nv, v in zip(rewards, dones, next_values, values)]
        deltas = np.stack(deltas) # in that particular case shorten values like from 0.39699999999999996 to 0.397
        gaes = copy.deepcopy(deltas)
        # get gae (update gae value)
        for t in reversed(range(len(deltas) - 1)):
            gaes[t] = gaes[t] + (1 - dones[t]) * gamma * lamda * gaes[t + 1] # lambda is smoothind parameter
        # calculate returns. Need to reverse the target list? (to obtain correct order from t=0 to t=127)
        target = gaes + values
        if normalize:
            gaes = (gaes - gaes.mean()) / (gaes.std() + 1e-8) # std - standart deviation
        return np.vstack(gaes), np.vstack(target) # vertical stack. from [1 2 3] to [[1][2][3]]
    ...
    def run(self): # train only when episode is finished
        ...
        while True:
            ...
            while not done:
                ...
                # print('##########')
                # print('env', len(self.env.step(action)))
                # print('env values', self.env.step(action))
...
if __name__ == "__main__":
    ...
    # agent.PlotModel(score, agent.episode)
    # agent.test()
