...
try:
    saveTime = 6/float(sys.argv[1])
    clocktick = 60*float(sys.argv[1])
except IndexError:
    saveTime = 6
    clocktick = 60
...
wnd_width = sw/2
wnd_height = sh/2
left_border = wnd_width/12
right_border = wnd_width - 2*left_border
...
# bot direction
population = [0, 1, 2]      # 0 - to the left(0.29 probab), 1 - to the right (0.29), 2 - stand still (0.42)
...
def eval_genomes(genomes, config):
    ...
    bot1 = Player(bot1Img)
    bot_direction = choices(population, weights)
    print('bot_direction = ', bot_direction)
    b1score = 0
    ...
    for genome_id, genome in genomes:
        ...
        aibuls.append(aibul)
        ge.append(genome)
        aibulcntr = Bullet(aibulcntrImg, -1, 4, ai.x+16, bot1.y+16)
    ...
    while run and len(ais) > 1 :
        ...
        if bot_direction == [1]:
            bot1.right()
            #print('moving right')
        elif bot_direction == [0]:
            bot1.left()
            #print('moving left')
        elif bot_direction == [2]:
            pass
        if bot1.x >= right_border:
            bot_direction = [0]
            #bot1.left()
        if bot1.x <= left_border:
            bot_direction = [1]
            #bot1.right()
        ...
        for xx, ai in enumerate(ais):
            ...
            if (bot1xnorm-axnorm) >= 0:
                ...
                direction = -1
            enemydist = sqrt( (axnorm-bot1xnorm)**2 + (aynorm-bot1ynorm)**2 ) * direction 
            if (aicntrbxnorm-axnorm) >= 0:
                buldirection = -1
            elif (aicntrbxnorm-axnorm) < 0:
                buldirection = 1
            aicntrbtot = sqrt(aicntrbxnorm**2 + aicntrbynorm**2)*buldirection
            
            #print('bot ',xx,' ', bot1xnorm)
            #print('ai ',xx,' ', axnorm)
            #print('enemy ',xx,' dist  ', enemydist)
            #print('bullet ',xx,' dist  ', aicntrbtot)
            ...
            output = nets[ais.index(ai)].activate((enemydist, aicntrbtot))
        ...
        # collide
        for xx, ai in enumerate(ais):
            #print('iteratingn through ais/collide check ', xx)
            '''
            ...
            try:
                if collide(ai, aibulcntrs[xx], bot1):
                    aibulcntrs[xx].y = bot1.y + 16      # bullets & ais are not lined after collide-killing
                    b1score += 1
                    ge[ais.index(ai)].fitness -= 1
                    #print('ai fitness ', ge[ais.index(ai)].fitness)
                    #print('collide-deleting NN ', xx, ' ', ais.index(ai)) 
                    ais.pop(xx)
                    nets.pop(xx)
                    ge.pop(xx)
                    aibulcntrs.pop(xx)
                ...
                print ("index error when removeing collided ai")
                pass
            except ValueError:
                print("value error when removing collided ai")
                pass
            try:
                if bot_direction != [2]:
                    #print('bot direction ', bot_direction)
                    if collide(bot1, aibuls[xx], ais[xx]):
                        aibuls[xx].y = ai.y - 16
                        aiscore += 1
                        ge[ais.index(ai)].fitness += 0.5
            except IndexError:
                print ("index error when +0.5 ge ai")
                pass
            except ValueError:
                print("value error when +0.5 ge ai")
                pass
        ...
        if deltTime > saveTime:
            ...
            print('current best (may be not saved) ', sortedfit[0][0], 'ship with ge.fitness=', sortedfit[0][1])
            try:
            ...
            try:
                print('deliting ', sortedfit[0][0], 'ship with ge.fitness=', sortedfit[len(sortedfit)-1][1])
                #makepause = input('enter to resume')
                ais.pop(sortedfit[0][0])
                nets.pop(sortedfit[0][0])
                ge.pop(sortedfit[0][0])
            except IndexError:
                pass
