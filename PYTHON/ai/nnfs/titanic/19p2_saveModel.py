...
# built atop RMSprop + momentum                                       |   EVALUATION here means
# with                                                                |   training, validation and testing
# decay = 1e-8  lr = 0.05  rho = 0.999  momentum = 0.9  epoch = 3.000 |   capabilities of the NN
# final result                                                        |_________________________________________
# accuracy = 0.980  loss = 0.07  lr = 0.047                           |
#                                                                     |   save weights and biases into .params file
#                                                                     |   And save whole model
import numpy as np
import random
import time
import os
...
class Model:
    ...
    def evaluate(self, X_val, y_val, *, batch_size=None):
        # de value if bath size is not being set
        validation_steps = 1
        # calculate number of steps
        if batch_size is not None:
            validation_steps = len(X_val) // batch_size
            # dividing round down. If there are some remaining
            # data, but not a full bath, this won't include it
            # Add '1' to include this not full batch
    ...
    def save_parameters(self, path):
        # open a file in a binary-write mode
        # and save params to it
    ...
    def save(self, path):
        ...
        # and gradients from the loss objeccts
        model.input_layer.__dict__.pop('output', None)
        model.loss.__dict__.pop('dinputs', None)
        # for eah layer remove inputs, output and dinputs properties
        for layer in model.layers:
            for property in ['inputs', 'output', 'dinputs', 'dweights', 'dbiases']:
                layer.__dict__.pop(property, None)
        # open a file in th binary-write mode and save the model
        with open(path, 'wb') as f:
            pickle.dump(model, f)
...
# add layers
model1.add(Layer_Dense(X.shape[1],256, weight_regularizer_l2=5e-4, bias_regularizer_l2=5e-4))
model1.add(Activation_Sigmoid())
model1.add(Layer_Dense(256,128))
model1.add(Activation_Sigmoid())
model1.add(Layer_Dropout(0.1))
model1.add(Layer_Dense(128,256))
model1.add(Activation_Sigmoid())
model1.add(Layer_Dropout(0.1))
model1.add(Layer_Dense(256,128))
model1.add(Activation_Sigmoid())
...
model1.set(loss=Loss_CategoricalCrossentropy(),optimizer=Optimizer_Adam(learning_rate=0.005, decay=1e-8), accuracy=Accuracy_Categorical())

# finalize the model
model1.finalize()

# train model
model1.train(X, y, epochs=5, batch_size=32, print_every=200, validation_data=(X_test, y_test))

print ('saving model...')
model1.save('fashion_mnist.model')
# new model
print ('loading model...')
