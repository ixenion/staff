...
# create dataset
X, y = create_sin_data(samples=512)
...
# add layers
model.add(Layer_Dense(1,64, weight_regularizer_l2=5e-4, bias_regularizer_l2=5e-4))
...
model.set(loss=Loss_MeanSquaredError(), optimizer=Optimizer_Adam(learning_rate=0.005, decay=1e-7, momentum=0.9, rho=0.999), accuracy=Accuracy_Regression())

# finalize the model
model.finalize()

# train model
model.train(X, y, epochs=4000, print_every=100)
